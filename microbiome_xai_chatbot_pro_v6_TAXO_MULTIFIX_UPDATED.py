...
- This inference is based on abundance changes across samples
    ...
    def network_null_significance(self, num_draws=200, random_state=42):
        ...
        p = float((np.sum(np.array(null_stats) >= obs) + 1) / (len(null_stats) + 1))  # permutation p (upper tail)
    ...
    def generate_network_visualization(self) -> Dict[str, Any]:
        ...
        fig = go.Figure(data=edge_traces + [node_trace],
                       layout=go.Layout(
                           ...
                           annotations=[ dict(
                               text="Hover over nodes for the long otu name.",
                               showarrow=False,
...
- "What families are present?"
    ...
    def handle_intelligent_taxonomy_query(self, df: pd.DataFrame, filename: str, query_lower: str) -> str:
        ...
        # We support both shapes and normalize to (parent_level, parent_name, child_level):
...
"""
    ...
    def lookup_taxonomy_info(self, search_term: str, query_lower: str) -> str:
        ...
        # A) "<rank?> <name> has|belongs to|is in which <rank>"
        m = re.search(
            r'\b(?:(kingdom|phylum|class|order|family|genus|species)\s+)?'   # optional source rank
            r'([a-z0-9_\-]+)\s+'                                             # taxon token
            r'(?:has|belongs?\s+to|is\s+in)\s+which\s+'                      # relation
            r'(kingdom|phylum|class|order|family|genus)\b',                  # target rank
            q
        )
        if m:
        ...
        # --- Legacy specific fallbacks (kept for backward compatibility) ---
    ...
    def _lookup_rank_generic(self, token, target_rank, source_rank=None) -> str:
        ...
        # find a taxonomy dataframe among uploaded CSVs
    ...
    # add this helper near _normcolmap / utils
    ...
    def _first_taxonomy_row_for_species(self, species: str):
        ...
        for fname, finfo in (self.uploaded_files.get("csv_files") or {}).items():
            ...
            if len(have) < 3:
                continue  # doesn’t look like a taxonomy table
    ...
    def _species_from_sequence(self, seq: str):
        ...
        for fname, finfo in (self.uploaded_files.get("csv_files") or {}).items():
            ...
            for col in df.columns:
                ...
                # loose contains as fallback (prefix)
        ...
        # 2) Try explicit mapping tables (otu_id -> species_name)
        for fname, finfo in (self.uploaded_files.get("csv_files") or {}).items():
            df = finfo.get("data")
            if df is None or not isinstance(df, pd.DataFrame):
                continue
...
def analyze_multiple():
    # Analyze multiple CSVs sent as JSON payload (`files`: list of {filename, otu_data}).
    # Stores all CSVs and returns a summary message. We do not hard-limit number/size.
    try:
...
def compare_csvs():
    ...
    try:
        ...
        def _string_set(series):
            # helper to coerce to trimmed string set
        ...
        numeric_shared = [col for col in shared if df_a[col].dtype in ['int64', 'float64', 'int32', 'float32'] and df_b[col].dtype in ['int64', 'float64', 'int32', 'float32']]
        statistics = {
        ...
        visualization_data = {
            'venn_diagram': {
                ...
                'total_unique': len(set_a | set_b) if 'set_a' in locals() and 'set_b' in locals() else 0
            },
            'correlation_summary': {
